...
_tstSectionWGServer() {
    ...
    _tst "wg show — интерфейс виден wg-утилите" \
        bash -c "wg show \"${SERVER_WG_NIC}\" 2>/dev/null | grep -q interface"
    _tst "UDP-порт сервера ${SERVER_PORT} прослушивается" \
        bash -c "ss -lnup 2>/dev/null | grep -q \":${SERVER_PORT} \""
    _tst "Публичный ключ сервера присутствует" \
        bash -c "wg show \"${SERVER_WG_NIC}\" public-key 2>/dev/null | grep -qE '^[A-Za-z0-9+/=]{43,44}$'"
    _tst "Есть хотя бы один настроенный пир" \
        bash -c "wg show \"${SERVER_WG_NIC}\" peers 2>/dev/null | grep -q ."
    _tst "IP-адрес интерфейса ${SERVER_WG_NIC} поднят" \
        bash -c "ip -4 addr show \"${SERVER_WG_NIC}\" 2>/dev/null | grep -q 'inet '"
    _tst_info "Пиров подключено (latest handshake < 3 мин)" \
        bash -c "wg show \"${SERVER_WG_NIC}\" latest-handshakes 2>/dev/null \
            | awk -v now=\"\$(date +%s)\" '\$2>0 && (now-\$2)<180{c++} END{print c+0\" активных\";}'"
    _tst_info "Версия WireGuard (wg --version)" \
        bash -c "wg --version 2>/dev/null | head -1"
}
...
_tstSectionDnsmasq() {
    ...
    _tst "dnsmasq не отвечает снаружи (lo only)" \
        bash -c "! ss -lntu 2>/dev/null | grep ':53' | grep -qv '127\.0\.0\.1\|::1'"

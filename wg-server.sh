...
repairRouting() {
    ...
    # [fix v28.24.17] Транзакция: снапшот только наших таблиц перед изменениями.
...
_rebuildTunnelForce() {
    ...
    if command -v _saveWgNftables >/dev/null 2>&1 && [ -f /etc/nftables.conf ]; then
        local _tmp
...
_telemt_downloadBinary() {
    ...
    # [fix v28.24.14] Раньше делали eval "${_prev_exit_trap}" — это могло
    # выполнить произвольный код, если строка trap'а содержала спецсимволы.
    # Теперь просто сбрасываем EXIT-trap: внешний код, который ставил свой
    # trap, повторно установит его сам при следующем входе в свою область.
    trap - EXIT
    unset _prev_exit_trap 2>/dev/null || true
}
...
showTelemetLinks() {
    ...
    port=$(grep '^port' "${TELEMT_CONFIG}" | head -1 | grep -oE '[0-9]+')

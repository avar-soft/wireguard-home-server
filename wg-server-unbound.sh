...
trap '_rc=$?; case "$_rc" in 0|130|141) :;; *)
  [ "${_WG_INTERACTIVE:-0}" = "1" ] && { _WG_INTERACTIVE=0; :; } && :; continue_log=0 || continue_log=1
  [ "${continue_log:-1}" = "0" ] && { :; } || {
  _src="${BASH_SOURCE[1]:-${BASH_SOURCE[0]:-main}}";
  _cmd="${BASH_COMMAND:-unknown}";
  case "${_cmd}" in read*|_pause*|ask*|askYesNo*) :;; *)
  _msg=$(printf "[%s] %s:%s:%s rc=%s cmd: %s" "$(date +%H:%M:%S)" "${_src}" "${FUNCNAME[0]:-main}" "$LINENO" "$_rc" "${_cmd}");
...
loadConfig() {
    ...
    # [fix v28.24.13] Жёсткая валидация имён интерфейсов — даже если в
    # конфиг попало что-то странное (ручная правка, повреждение), мы
    # отказываемся работать вместо того чтобы передать это в bash -c / sed.
    # Снимает риск shell-injection в _test/ExecStartPre/sed -i подстановках.
    _validIfaceName() {
        local n="$1"
...
# [fix IPv6 regex] Валидация IPv6 CIDR через ядерный парсер вместо regex.
# Regex вида ^[0-9a-fA-F:]+/[0-9]{1,3}$ пропускает невалидные адреса (:::::::/999).
# Используем ip route blackhole — ядро сообщит об ошибке если адрес невалиден.
validate_ipv6_cidr() {
    local cidr="$1"
    [[ "${cidr}" == *:*/* ]] || return 1
    ip -6 route add blackhole "${cidr}" 2>/dev/null || return 1
    ip -6 route del blackhole "${cidr}" 2>/dev/null || true
    return 0
}
...
MTU = ${SERVER_MTU:-1420}
Table = off
PostUp = ip -4 rule add fwmark 0x200000 table main priority 30050 || true
PostDown = ip -4 rule del fwmark 0x200000 table main priority 30050 2>/dev/null || true; ip -4 rule del fwmark 0x100000 priority 30045 2>/dev/null || true
EOF
...
_setupNetworkManagerUnmanaged() {
    if ! command -v nmcli >/dev/null 2>&1 && ! [ -d /etc/NetworkManager ]; then
        # NM не установлен — нечего настраивать
        return 0
    fi
...
_rebuildTunnelForce() {
    ...
    if { [ "${_have_v4}" = "1" ] && [ "${#_v4[@]}" -gt 0 ]; } || \
       { [ "${_have_v6}" = "1" ] && [ "${#_v6[@]}" -gt 0 ]; }; then
        ...
        trap 'rm -f "${_nft_batch}" 2>/dev/null' RETURN
        local _CHUNK=200
...
_dotEdit() {
    ...
    command -v "${_ed}" >/dev/null 2>&1 || _ed="vi"
...
_txn_begin() {
    ...
    local _t
    for _t in wg-policy wg-filter wg-nat; do
        # Добавляем idempotent-заголовок: создать если нет, затем сбросить
        printf 'add table inet %s\ndelete table inet %s\n' "${_t}" "${_t}" >> "${_WG_TXN_SNAPSHOT}" 2>/dev/null || true
...
firstInstall() {
    ...
    # [fix КРИТИЧ 1] Строгая повторная валидация SERVER_PUB_IP перед сохранением.
    # Защита от инъекции: допускаем ТОЛЬКО корректный IPv4 (4 октета 0-255).
    if ! [[ "${SERVER_PUB_IP}" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then
    ...
    for _oct in "${_o1}" "${_o2}" "${_o3}" "${_o4}"; do
        [[ "${_oct}" =~ ^[0-9]+$ ]] && [ "${_oct}" -le 255 ] ||             error "Недопустимый октет в SERVER_PUB_IP: '${_oct}'"
    done
...
/var/log/wg-server-trap.log {
    ...
    postrotate
        systemctl kill -s HUP wg-balance.service 2>/dev/null || true
    endscript
}
LOGROTATE
    ...
    # Корневые серверы меняются редко, но раз в полгода обновление нужно
    (crontab -l 2>/dev/null | grep -v "wg-root-hints" || true
     echo "0 3 1 * * curl -fsSL --max-time 15 https://www.internic.net/domain/named.cache -o /var/lib/unbound/root.hints 2>/dev/null && systemctl reload-or-restart unbound 2>/dev/null || true  # wg-root-hints") | crontab -
    ...
    while true; do
        ...
        case "${opt}" in
            1)  # Защитить offline файлы
                ...
                [ -f "${GEO_V4}" ] && _geoSetReadonly "${GEO_V4}" || warn "IPv4 offline файл не найден"
                [ -f "${GEO_V6}" ] && _geoSetReadonly "${GEO_V6}" || warn "IPv6 offline файл не найден"
                ;;
            2)  # Разрешить запись offline
                local _ans
                read -rp "  Разрешить запись в offline файлы (chattr -i)? [y/N]: " _ans
                [[ "${_ans}" =~ ^[Yy]$ ]] || { warn "Отменено"; echo ""; read -rp "  [Enter]..." _d; continue; }
                [ -f "${GEO_V4}" ] && _geoSetWritable "${GEO_V4}" || warn "IPv4 offline файл не найден"
                [ -f "${GEO_V6}" ] && _geoSetWritable "${GEO_V6}" || warn "IPv6 offline файл не найден"
                ;;
...
_telemt_downloadBinary() {
    ...
    # Один trap без снятия гарантирует очистку при любом выходе.
...
runSystemTest() {
    ...
    # [fix v28.24.19] Убираем bash -c из _test() — архитектурно небезопасный
    # подход (shell injection через имена интерфейсов/хостов даже после валидации).
    # Каждая проверка оформлена как именованная функция с явными аргументами —
    # никакой генерации строк, никакого eval/bash -c.
    _test() {
        local desc="$1"; shift
        printf "  %-55s" "${desc}"
    ...
    # ── Именованные функции-проверки (без bash -c / eval) ──────────
    _chk_wg_iface()    { wg show "$1" 2>/dev/null | grep -q 'interface'; }
    _chk_svc_active()  { systemctl is-active "$1" 2>/dev/null; }
    _chk_nft_policy()  { nft list table inet wg-policy >/dev/null 2>&1; }
    _chk_tunnel_up()   { wg show "$1" 2>/dev/null | grep -q 'interface'; }
    _chk_host_reach()  {
        local _host="$1" _iface="$2"
        ping -c 1 -W 3 "${_host}" >/dev/null 2>&1 \
            || wg show "${_iface}" 2>/dev/null | grep -q 'latest handshake'
    }
    _chk_fwmark()      { ip rule show 2>/dev/null | grep -q 'fwmark 0x200000 lookup main'; }
    _chk_balance_tbl() {
    ...
    _chk_geoip4() {
        local _geo_count
        _geo_count=$(nft list set inet wg-policy russia 2>/dev/null \
            | grep -oE '[0-9]{1,3}(\.[0-9]{1,3}){3}/[0-9]{1,2}' | wc -l)
        if [ "${_geo_count}" -le 100 ] \
           && [ -s /etc/wireguard/geoip/ru-aggregated.zone ] \
           && [ -x /usr/local/bin/update-ru-ipset.sh ]; then
    ...
    _chk_geoip6() {
        # Мягкая проверка: если set отсутствует — OK.
        # FAIL только когда offline-файл есть, но set пуст.
        if ! nft list set inet wg-policy russia_v6 >/dev/null 2>&1; then
            return 0
        fi
        local _g6
        _g6=$(nft list set inet wg-policy russia_v6 2>/dev/null \
              | grep -oEc '[0-9a-fA-F:]+/[0-9]+')
        _g6=${_g6:-0}
        if [ -s /etc/wireguard/geoip/ru-aggregated-v6.zone ]; then
            [ "${_g6}" -gt 0 ]
        else
    ...
    _chk_offline_geoip4() { [ -s /etc/wireguard/geoip/ru-aggregated.zone ]    || return 0; }
    _chk_offline_geoip6() { [ -s /etc/wireguard/geoip/ru-aggregated-v6.zone ] || return 0; }
    _chk_systemd_units()  {
        systemd-analyze verify \
            "wg-quick@${SERVER_WG_NIC}.service" \
            wg-balance.service 2>/dev/null
    }
    _chk_ping_8888() {
        ping -c 1 -W 3 8.8.8.8 >/dev/null 2>&1 \
            || ( exec 3<>/dev/tcp/8.8.8.8/53 ) 2>/dev/null
    }
    _chk_dns_google()  { getent hosts google.com >/dev/null 2>&1; }
    _chk_dns_chain_geo()    { dig @127.0.0.1 -p 5335 google.com +short +time=3 >/dev/null 2>&1; }
    _chk_dns_chain_stubby() { dig @127.0.0.1 -p 5353 google.com +short +time=3 >/dev/null 2>&1; }
    _chk_dnsmasq_port()     { ss -lntu 2>/dev/null | grep -q '127\.0\.0\.1:53\b'; }

    echo -e "  ${WHITE}── Сервисы ──${NC}"
    _test "WireGuard сервер (${SERVER_WG_NIC}) активен"   _chk_wg_iface   "${SERVER_WG_NIC}"
    _test "Балансировщик wg-balance активен"              _chk_svc_active "wg-balance"
    # nftables — проверяем что таблица wg-policy существует в ядре
    _test "nftables правила загружены"                    _chk_nft_policy
    [ -x "${TELEMT_BIN:-/usr/local/bin/telemt}" ] && \
        _test "Telemt MTProxy активен"                    _chk_svc_active "telemt"

    echo ""
    echo -e "  ${WHITE}── Туннели ──${NC}"
    local i
    for ((i=0; i<TUNNEL_COUNT; i++)); do
        _test "Туннель ${TUNNEL_IFACE[$i]} активен"  _chk_tunnel_up "${TUNNEL_IFACE[$i]}"
        local host="${TUNNEL_ENDPOINT[$i]%%:*}"
        _test "Связь с сервером ${host}"             _chk_host_reach "${host}" "${TUNNEL_IFACE[$i]}"
    done

    echo ""
    echo -e "  ${WHITE}── Маршрутизация ──${NC}"
    # fwmark: PostUp в wg0.conf добавляет правило. Проверяем через ip rule и через wg show.
    _test "fwmark правило РФ трафика"                     _chk_fwmark
    # Таблицы балансировщика: iif wg0 lookup 51821/51822
    _test "Балансировщик активен (таблицы)"               _chk_balance_tbl
    ...
    _test "GeoIP база РФ (8500+ сетей) загружена"         _chk_geoip4
    ...
    # [fix v28.24.9] Если set существует — тест проходит. FAIL только когда
    # есть offline-файл с данными, но set пуст (значит загрузка не отработала).
    _test "GeoIP IPv6 (@russia_v6) загружена или отсутствует" _chk_geoip6
    # [v28.22.0] Информативная проверка наличия offline-файлов GeoIP (никогда не FAIL)
    _test "Offline GeoIP IPv4 (опционально)"               _chk_offline_geoip4
    _test "Offline GeoIP IPv6 (опционально)"               _chk_offline_geoip6
    # [fix v28.20.8] Валидация systemd unit-файлов — ловит синтаксические ошибки в drop-in
    _test "systemd units валидны (verify)"                 _chk_systemd_units

    echo ""
    echo -e "  ${WHITE}── Интернет ──${NC}"
    # Ping: на VDS хостеры иногда блокируют ICMP — TCP fallback на порт 53
    _test "Ping / связь 8.8.8.8"                          _chk_ping_8888
    _test "DNS резолвинг google.com"                      _chk_dns_google
    [ "${DNS_MODE}" = "geo" ]    && _test "unbound :5335 отвечает"  _chk_dns_chain_geo
    [ "${DNS_MODE}" = "tunnel" ] && _test "stubby :5353 отвечает"   _chk_dns_chain_stubby
    [ "${DNS_MODE}" != "public" ] && _test "dnsmasq слушает :53"    _chk_dnsmasq_port
    ...
    case "${choice}" in
        ...
        2)
            ...
            if install -m 0755 "${LOCAL_FILE}" "${SCRIPT_PATH}.new" \
               && mv -f "${SCRIPT_PATH}.new" "${SCRIPT_PATH}"; then
                info "Скрипт обновлён из ${LOCAL_FILE} (атомарно)."
                warn "Перезапусти: ${SCRIPT_PATH}"
            else
...
menu() {
    ...
    while true; do
        ...
        echo -e "  ${YELLOW} 7${NC})  🌐  ${WHITE}DNS (geo/tunnel/pub)${NC}  ${DIM}— Яндекс напрямую или DoT (Cloudflare/Quad9) через туннель${NC}"

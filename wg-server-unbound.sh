#!/bin/bash
# ╔══════════════════════════════════════════════════════════════╗
# ║   WireGuard Home Server — Версия 28.24.21                    ║
...
trap '_rc=$?; case "$_rc" in 0|130|141) :;; *)
  ...
  _msg=$(printf "[%s] %s:%s:%s rc=%s cmd: %s" "$(date +%H:%M:%S)" "${_src}" "${FUNCNAME[0]:-main}" "$LINENO" "$_rc" "$BASH_COMMAND");
...
safe_sed() {
    ...
    # [fix v28.25.0] Экранируем ВСЕ regex-метасимволы в LHS (. * [ ] ^ $ { } + ? и т.д.),
    # а не только '|' — иначе сложные LHS с . или * дают непредсказуемые замены.
...
loadConfig() {
    ...
    _validIfaceName() {
        local n="$1"
        # IFNAMSIZ=15 в ядре Linux; разрешаем только [A-Za-z0-9_-], начало с буквы.
...
enableForwarding() {
    ...
    # [fix v28.25.0] Выставляем rp_filter=2 явно на каждом интерфейсе —
    # max(all.rp_filter, iface.rp_filter) может оставить iface на =1 (strict),
    # что блокирует пакеты из туннеля на хостах с жёсткими /etc/sysctl.d/*.conf.
...
EOF
    ...
    rm -f /etc/resolv.conf
    # [fix v28.23.2]  #1: компромисс между утечкой DNS и полной потерей DNS.
...

        {
            if [ "${_have_v4}" = "1" ] && [ "${#_v4[@]}" -gt 0 ]; then
                ...
                for ((_i=0; _i<${#_v4[@]}; _i+=_CHUNK)); do
                    ...
                    _emit_elements "force_tunnel_v4" "${_slice[@]}"
                done
            fi
            if [ "${_have_v6}" = "1" ] && [ "${#_v6[@]}" -gt 0 ]; then
                ...
                for ((_j=0; _j<${#_v6[@]}; _j+=_CHUNK)); do
                    ...
                    _emit_elements "force_tunnel_v6" "${_slice6[@]}"
                done
            fi
        } > "${_nft_batch}"
...
_rebuildDirectDomains() {
    ...
    {
        echo "# Домены напрямую мимо VPN (форвард на Яндекс.DNS 77.88.8.8 — в @russia)"
        ...
        while IFS= read -r line; do
            ...
            if echo "${line}" | grep -qE '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+(/[0-9]+)?$'; then
                ...
                # Домен → форвардим на Яндекс.DNS (IP в @russia → трафик напрямую)
                echo "server=/${line}/77.88.8.8"
                echo "server=/${line}/77.88.8.1"
            fi
...
_txn_step() {
    ...
    # Захватываем stderr для логирования, не теряем его
    local _step_err
    if _step_err=$(bash -c "$*" 2>&1); then
        return 0
    else
...
_telemt_downloadBinary() {
    ...
    local tmpdir
    tmpdir=$(mktemp -d)
    # [fix v28.25.0] Используем подоболочку вместо сохранения/восстановления EXIT-trap.
    # trap внутри ( ... ) локален для подоболочки и не затрагивает родителя.
    (

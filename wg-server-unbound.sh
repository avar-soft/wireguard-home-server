...
createConfigs() {
    ...
    for ((i=0; i<TUNNEL_COUNT; i++)); do
        ...
        # [fix] umask 077 — конфиг с PrivateKey создаётся сразу без гонки chmod
        (umask 077; cat > "/etc/wireguard/${iface}.conf" << EOF
[Interface]
...
LOGROTATE
    ...
    (crontab -l 2>/dev/null | grep -v "wg-root-hints" || true
     echo "30 3 1 * * curl -fsSL --max-time 15 https://www.internic.net/domain/named.cache -o /var/lib/unbound/root.hints 2>/dev/null && systemctl reload-or-restart unbound 2>/dev/null || true  # wg-root-hints") | crontab -
